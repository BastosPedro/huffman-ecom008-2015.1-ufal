...
{
    qDebug() << endl << endl << "Representing the tree: " << endl << qPrintable(anyTree->getRepresentation());
}

void auxilia::printFrequency(compression *&anyFile)
{
    qDebug() << endl << endl;
...
void auxilia::printBitString(compression *&anyFile)
{
...
void auxilia::printAll(tree* anyTree, node* anyNode, compression*& anyFile){
    printList(anyTree);
    printHash(anyTree);
